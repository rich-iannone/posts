...
For advanced YAML features (tagged values, complex mapping keys, multi-document streams), `py-yaml12` provides the `Yaml` wrapper type, but these are opt-in and unnecessary for typical configuration work:
...
In the meantime, you can `pip install py-yaml12` and see what a YAML 1.2 implementation looks like in Python. The Norway problem is solved. The specification is clear. The tooling has caught up.
...
[^17]: "[CVE-2017-18342](https://nvd.nist.gov/vuln/detail/CVE-2017-18342)", National Vulnerability Database. CVSS 9.8 Critical: arbitrary code execution via `yaml.load()` with untrusted data in PyYAML before 5.1. See also PyYAML wiki, "[PyYAML yaml.load(input) Deprecation](https://github.com/yaml/pyyaml/wiki/PyYAML-yaml.load(input)-Deprecation)".
